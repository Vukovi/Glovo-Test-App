    ...
    public static func setObjectValue<T>(_ anyObject: T, key: String, theValue: Any?, typeInObject: String? = nil, valid: Bool, conversionOptions: ConversionOptions = .DefaultDeserialize, parents: [NSObject] = []) where T: NSObject {
        ...
        if typeInObject == "Struct" {
            ...
            do {
                ...
                (anyObject as? EVReflectable)?.addStatusMessage(.InvalidValue, message: "Not a valid value for object `\(NSStringFromClass((anyObject as AnyObject as! AnyClass)))`, type `\(type)`, key  `\(key)`, value `\(value)`")
                evPrint(.InvalidValue, "INFO: Not a valid value for object `\(NSStringFromClass((anyObject as AnyObject as! AnyClass)))`, type `\(type)`, key  `\(key)`, value `\(value)`")
            }
